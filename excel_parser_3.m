...
%
% Purpose: This program receives an experimental folder name ('3 barcodes'),
% iterates through all sub-directories, i.e., the repeated experiment 3 BC 
% folders ('17XXXX_HMS_XX_xxxxxxx_XXXXXXXXX') and collects categories of
...
% sorted by number of "correct_level_count_pore" pores at start ("Sort 
% Largest to Smallest). Use MATLAB R2017a to run this code, older version 
...
for exp = 1:length(list) 
    ...
    for coi_id = 1:length(coi)
        ...
        legend([regexprep(exp_dir, '_', '-'), ' (N = ', num2str(seq_pores), ')'], 'Location', 'northoutside');

        % Save plot.
        savefig([num2str(coi_id), '_', coi_name, '.fig']);
        print('-dbmp', [num2str(coi_id), '_', coi_name, '.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+1), '_read_length_hist.fig']);
    print('-dbmp', [num2str(length(coi)+1), '_read_length_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+2), '_single_pore_hist.fig']);
    print('-dbmp', [num2str(length(coi)+2), '_single_pore_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+3), '_ttt_all_hist.fig']);
    print('-dbmp', [num2str(length(coi)+3), '_ttt_all_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+4), '_ttt_c_hist.fig']);
    print('-dbmp', [num2str(length(coi)+4), '_ttt_c_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+5), '_ttt_a_hist.fig']);
    print('-dbmp', [num2str(length(coi)+5), '_ttt_a_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+6), '_ttt_t_hist.fig']);
    print('-dbmp', [num2str(length(coi)+6), '_ttt_t_hist.bmp']);
    ...
    % Save plot.
    savefig([num2str(length(coi)+7), '_ttt_g_hist.fig']);
    print('-dbmp', [num2str(length(coi)+7), '_ttt_g_hist.bmp']);
    ...
    cells_list.experiment(exp).s = cells;
...
save('pol-bar_mixing_stats_3.mat', 'cells_list');
...
% Move all figures to 'mat_files' directory.
movefile('*stats_3*', 'mat_files');

% Navigate to working directory.
cd(work_dir);

fprintf('\n');
disp('--> EXCEL parser 3 end');
fprintf('\n');
