...
% errors. Fix 'pol', vary 'bar'.
%
% This software may be used, modified, and distributed freely, but this
% header may not be modified and must appear at the top of this file.
%--------------------------------------------------------------------------

function whisker_pol
...
fprintf('\n');
disp('--> Whisker polymerase fix start');
fprintf('\n');
...
for e = 1:length(list) 
    ...
    for coi_id = 1:length(coi)
        ...
        grid;
        title([ordinate_name, ' pol-whisker']);
        xlabel('pol id');
        ...
        % Save plot.
        pp = strsplit(ss{1}, '_');
        fn = [num2str(coi_id), '_', pp{1}, '_', coi_name];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Whisker polymerase fix end');
fprintf('\n');
