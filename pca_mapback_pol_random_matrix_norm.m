...
function pca_mapback_pol_random_matrix_norm(type, R)
...
fprintf('\n');
disp('--> PCA map back random matrix normalized start');
fprintf('\n');
...
% Random (N) pore selection from particular set.
[r, c] = size(BAR);
range = [1 r];
RAND = [];

for n = 1:R
    RAND = vertcat(RAND, BAR(randi(range), :));
end

% Calculate principle components of data matrix.
opt = statset('pca'); opt.MaxIter = 1e+6;
[coeff, score, latent, tsquared, explained, mu] = pca(RAND, 'Algorithm', 'svd', 'Options', opt);
P = RAND*coeff;
PC1 = P(:,1); PC2 = P(:,2);
...
dlmwrite('pca_matrix.txt', RAND, 'delimiter', '\t', 'precision', '%.4f');

disp('--> PROCESSED DATA SET: data_matrix.txt'); 

% Save plot.
savefig([num2str(type), '_pca_mapback_pol_random.fig']);
print('-dbmp', [num2str(type), '_pca_mapback_pol_random.bmp']);
...
fprintf('\n');
disp('--> PCA map back random matrix normalized end');
fprintf('\n');
