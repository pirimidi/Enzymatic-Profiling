...
function recursive_naive_barcodes_nw_70(num_bar, ran_bar, threshold)
...
fprintf('\n');
disp('--> Recursive naive barcodes NW 70 start');
fprintf('\n');
...
while(length(barcodes) ~= num_bar+1)
    ...
    fastawrite('unique_barcodes_nw_70.fasta', bc_data(bc));
...
disp('--> Recursive naive barcodes NW 70 end');
fprintf('\n');
