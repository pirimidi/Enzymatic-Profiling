...
function recursive_naive_barcodes(num_bar, ran_bar, threshold)
...
fprintf('\n');
disp('--> Recursive naive barcodes start');
fprintf('\n');
...
while(length(barcodes) ~= num_bar)
    ...
    [barcodes, scores] = barcode_check(barcodes, ran_bar, threshold, scores);
...
disp('--> Recursive naive barcodes end');
fprintf('\n');
