...
function barcode_unique_count_96(x_min, x_max, y_min)
...
fprintf('\n');
disp('--> Barcode unique count 96 start');
fprintf('\n');
...
% Iterate through all MAT-files (currently only one).
 for e = 1:length(list) 
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        % Filter out unique barcodes if more then one identified for a
        % single pore.
        [unique_barcode_id, id, ie] = unique(bc);
        ...
        histogram(bc, 96)
        title('unique barcode distribution');
        xlabel('barcode id');
        ylabel('count');
        y_lim = ylim; axis([0 97 0 y_lim(2)+1]);
        xticks(1:96);
        xtickangle(45);
        legend(['pores = ', num2str(length(unique_cell_id)), ...
                ' > barcodes = ', num2str(length(unique_barcode_id)), ...
                ' > total counts = ', num2str(length(bc))]);
        figureFullScreen(counter);

        % Save plot.
        fn = [num2str(exp), '_', 'bar_unique_count_96'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Barcode unique count 96 end');
fprintf('\n');
