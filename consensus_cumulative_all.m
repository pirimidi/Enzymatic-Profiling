...
function consensus_cumulative_all(iter_cutoff)
...
fprintf('\n');
disp('--> Consensus cumulative all start');
fprintf('\n');
...
cdir = 'consesnsus_cumulative_all';
    ...
    for bar = 1:length(barcodes)  
        ...
        for exp = 1:length(cons_list(bar).experiment)
            ...
            for m = start:cutoff
                ...
                % Find iteration values cumulatively.
        ...
        grid;
        title('consensus identity iteration-cumulative-all-cons');
...
disp('--> Consensus cumulative all end');
fprintf('\n');
