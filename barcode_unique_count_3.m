...
function barcode_unique_count_3(x_min, x_max, y_min)
...
fprintf('\n');
disp('--> Barcode unique count 3 start');
fprintf('\n');
...
% Iterate through all MAT-files (currently only one).
 for e = 1:length(list) 
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        % Save plot.
        fn = [num2str(e), '-', num2str(exp), '_bar_unique_count_3'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Barcode unique count 3 end');
fprintf('\n');
