...
% the experimental structures and generates whisker plots featuring 
% consesnsus alignment accuracy (%) for each 'pol-bar' case displaying them
% on the same plot for comparison. NOTE: Use MATLAB R2017a to run this 
...
coi = {'iteration', 'alignment_length', 'cons_alignment_length', ...
       'match_count', 'mismatch_count', 'insertion_count', 'deletion_count', 'identity', ...
       'cons_match_count', 'cons_mismatch_count', 'cons_insertion_count', 'cons_deletion_count', 'cons_identity',};
...
for bar = 1:length(barcodes)
    ...
    for coi_id = 1:length(coi)
        ...
        grid;
        title([ordinate_name, ' cons-whisker plot']);
        xlabel('pol id');
