...
function barcode_identity_map_3_filter(x_min, x_max, y_min, c_min)
...
fprintf('\n');
disp('--> Barcode identity map 3 filter start');
fprintf('\n');
...
cdir = 'barcode_identity_map_3_filter';
 ...
 for e = 1:length(list) 
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        for i = 1:length(unique_cell_id)
            ...
            % Iterate through all 3 barcodes and build M column-by-column.
        ...
        % Save plot.
        fn = [num2str(e), '-', num2str(exp), '_bar_id_map_3_filter'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Barcode identity map 3 filter end');
fprintf('\n');
