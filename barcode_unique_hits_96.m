...
function barcode_unique_hits_96(x_min, x_max, y_min)
...
fprintf('\n');
disp('--> Barcode unique hits 96 start');
fprintf('\n');
...
% Iterate through all MAT-files (currently only one).
 for e = 1:length(list) 
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        % Generate figure for each experiment.
        figure(counter);

        % Create dwell time (s) boxplot of all filtered events (exp).
        CategoricalScatterplot(bc, ge, 'Marker', 'o', 'FillMarker', true);
        grid;
        title('unique barcode id vs. pore id');
        xlabel('pore id');
        ylabel('barcode id');
        ylim([0 97]);
        yticks(1:96);
        xtickangle(45);
        legend(['pores = ', num2str(length(unique_cell_id))], ['identified barcode = ', num2str(length(bc))]);
        figureFullScreen(counter);

        % Save plot.
        fn = [num2str(exp), '_', 'bar_unique_hits_96'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Barcode uniquehits 96 end');
fprintf('\n');
