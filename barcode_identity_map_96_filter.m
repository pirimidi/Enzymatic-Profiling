...
function barcode_identity_map_96_filter(x_min, x_max, y_min, c_min)
...
fprintf('\n');
disp('--> Barcode identity map 96 filter start');
fprintf('\n');
...
cdir = 'barcode_identity_map_96_filter';
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        for bar = 1:length(bar_seqs) 
            ...
            if length(pb_f3) > 1
                ...
                i_4 = extractfield(pb_f3, 'cons_alignment_length') >= c_min;   
                pb_f4 = pb_f3(i_4); 
                ...
                if length(pb_f4) > 1
                    ...
                    % Find 'coi' values.
                    norm = extractfield(pb_f4, 'cons_identity')./100;
                    cell_id = extractfield(pb_f4, 'cell_id');
        ...
        % Save plot.
        fn = [num2str(e), '-', num2str(exp), '_bar_id_map_96_filter'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Barcode identity map 96 filter end');
fprintf('\n');
