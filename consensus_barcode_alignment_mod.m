...
% the experimental structures and generates a consensus alignment of the 
% barcoded region only (32 bp) for each nanopore read, then determines a 
% error profile for each ('pol-bar') consensus. Finally, it generates 
% whisker plots featuring consensus alignment accuracy (%) versus number of
% full barcode iteration ("align_copies") for each 'pol-bar' case 
...
function consensus_barcode_alignment_mod
...
fprintf('\n');
disp('--> Consensus barcode alignment start');
fprintf('\n');
...
bar_seqs = fastaread('bar_list.fas');
...
for e = 1:length(list) 
        ...
        for exp = 1:length(cons_list(bar).experiment)
            ...
            if length(cons_list(bar).experiment(exp).s) > 1
                ...
                for o = 1:length(pb)
                    ...
                    barcode = template(6:length(template)-7);
                    ...
                    if iscell(frag)
                        ...
                        [c_score, c_alignment, c_start] = swalign(barcode, lumped_cseq);
                        ...
                        if length(c_alignment) >= 10
                            ...
                            [c_score_3, c_alignment_3, c_start_3] = swalign(lumped_template_3, lumped_cseq);
    ...
    pp = strsplit(ss{1}, '_');
    save([pp{1}, '_bar_cons_align.mat'], 'cons_list');
...
fprintf('\n');
disp('--> Consensus barcode alignment end');
fprintf('\n');
