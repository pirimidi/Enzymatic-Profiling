...
function consensus_cumulative_stop_all
...
fprintf('\n');
disp('--> Consensus cumulative stop all start');
fprintf('\n');
...
cdir = 'consensus_cumulative_stop_all';
...
% Iterate through all 'pol' folders.
 for e = 1:length(list) 
    ...
    for bar = 1:length(barcodes)  
        ...
        grid;
        title('consensus identity cumulative stop all');
        ...
        % Save plot.
        pp = strsplit(ss{1}, '_');
        fn = [pp{1}, '_', barcodes{bar}, '_cumu_stop_all'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
disp('--> Consensus cumulative stop all end');
fprintf('\n');
