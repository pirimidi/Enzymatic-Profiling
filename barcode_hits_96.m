...
function barcode_hits_96(x_min, x_max, y_min)
...
% Iterate through all MAT-files (currently only one).
 for e = 1:length(list) 
    ...
    for exp = 1:length(cons_list(1).experiment)
        ...
        grid;
        title('barcode id vs. pore id');
        xlabel('pore id');
        ylabel('barcode id');
        ylim([0 97]);
        yticks(1:96);
        xtickangle(45);
        legend(['pores = ', num2str(length(unique(GE)))], ['potential barcodes = ', num2str(length(BC))]);
        figureFullScreen(counter);

        % Save plot.
        fn = [num2str(exp), '_', 'bar_hits_96'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
