...
function recursive_naive_barcodes_homo(num_bar, ran_bar, threshold)
...
fprintf('\n');
disp('--> Recursive naive barcodes HOMO start');
fprintf('\n');
...
barcodes{1} = [l randseq(ran_bar) r];
...
while(length(barcodes) ~= num_bar)
    ...
    barcodes{length(barcodes)+1} = [l randseq(ran_bar) r];
    [barcodes, scores] = barcode_check_homo(barcodes, ran_bar, threshold, scores);
...
fastawrite('unique_barcodes_homo.fasta', bc_data);

disp('--> Recursive naive barcodes HOMO end');
fprintf('\n');
