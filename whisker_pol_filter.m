...
% for each filtered 'pol-bar' case displaying them on the same plot for 
...
function whisker_pol_filter(x_min, x_max, y_min)
...
fprintf('\n');
disp('--> Whisker polymerase fix filter start');
fprintf('\n');
...
for e = 1:length(list) 
    ...
    for coi_id = 1:length(coi)
        ...
        % Define array container for fixed 'pol'.
        POL = [];
            
        % Iterate through all three barcodes.
        for bar = 1:length(merge_list)
            ...
            % Generate container array for 'pol-bar' combination (fix pol).
            POL = [POL; COI];
        ...
        %boxplot(POL, GE, 'PlotStyle', 'compact', 'OutlierSize', 1, 'Whisker', 0.2413);
        grid;
        title(['filtered ', ordinate_name, ' pol-whisker']);
        xlabel('bar id');
        ...
        % Save plot.
        pp = strsplit(ss{1}, '_');
        fn = [num2str(coi_id), '_', pp{1}, '_', coi_name, '_filter'];
        savefig([fn, '.fig']);
        print('-dbmp', [fn, '.bmp']);
...
fprintf('\n');
disp('--> Whisker polymerase fix filter end');
fprintf('\n');
