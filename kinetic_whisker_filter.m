...
function kinetic_whisker_filter(x_min, x_max, y_min)
...
fprintf('\n');
disp('--> Kinetic whisker filter start');
fprintf('\n');
...
for bar = 1:length(barcodes)
    ...
    cdir = ['kinetic_whisker_filter_', barcodes{bar}];
...
fprintf('\n');
disp('--> Kinetic whisker filter end');
fprintf('\n');
